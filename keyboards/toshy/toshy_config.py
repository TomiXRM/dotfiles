...
USER_IME_ENGINE_EN = os.environ.get('TOSHY_IME_ENGINE_EN', 'keyboard-us')
USER_IME_ENGINE_JP = os.environ.get('TOSHY_IME_ENGINE_JP', 'mozc')


def _switch_input_engine(engine_name: str):
    """Switch the active input engine via ibus (fallback: fcitx5)."""
    if not engine_name:
        error('Input-engine name is empty. Update USER_IME_ENGINE_* in toshy_config.')
        return

    try:
        subprocess.Popen(['ibus', 'engine', engine_name], stdout=DEVNULL, stderr=DEVNULL)
        return
    except FileNotFoundError:
        # IBus is not available – try fcitx5 instead (if installed)
        fcitx_remote = shutil.which('fcitx5-remote')
        if fcitx_remote:
            try:
                subprocess.Popen([fcitx_remote, '-s', engine_name], stdout=DEVNULL, stderr=DEVNULL)
                return
            except Exception as exc:
                error(f"fcitx5-remote failed to switch engine '{engine_name}': {exc}")
                return
        error("Could not run 'ibus'. Install it or adjust _switch_input_engine().")
        return
    except Exception as exc:
        error(f"Failed to switch input engine to '{engine_name}': {exc}")


def switch_to_english_ime():
    """Switch to the configured English/Latin input engine."""
    _switch_input_engine(USER_IME_ENGINE_EN)


def switch_to_japanese_ime():
    """Switch to the configured Japanese IME engine."""
    _switch_input_engine(USER_IME_ENGINE_JP)
...
#     Key.CAPSLOCK:               [Key.ESC, Key.LEFT_CTRL]       # Caps2Esc - not Chromebook
# }, when = lambda ctx:
...
# [Global Terminals conditional modmaps] Change modifier keys in terminals
modmap("Cond modmap - Terms - Caps2Cmd - not Cbk kdb", {
    Key.CAPSLOCK:               Key.LEFT_CTRL,                 # Caps2Cmd (terminals)
}, when = lambda ctx:
    cnfg.Caps2Cmd and
    cnfg.screen_has_focus and
    not isKBtype('Chromebook')(ctx) and
...
modmap("Cond modmap - Terms - Caps2Cmd - Cbk kdb", {
    Key.LEFT_META:              Key.RIGHT_CTRL,                # Caps2Cmd - Chromebook (terminals)
}, when = lambda ctx:
    cnfg.Caps2Cmd and
    cnfg.screen_has_focus and
    isKBtype('Chromebook')(ctx) and
...
modmap("User - Win kbd - Meta to Alt", {
    # Remap Windows/Super (Meta) keys to Alt on Windows-type keyboards
...
multipurpose_modmap("JIS IME dual-role controls(Windows)", {
    # Muhenkan (left of space) acts as English toggle on tap, Ctrl when held
    Key.LEFT_ALT:              [Key.MUHENKAN, Key.RIGHT_CTRL],
    # Henkan (right of space) acts as Japanese toggle on tap, Ctrl when held
...
keymap("Angry IP Scanner", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^Angry.*IP.*Scanner$")(ctx) )
...
keymap("Overrides for COSMIC Files - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^com.system76.CosmicFiles$")(ctx) )
...
keymap("Overrides for Dolphin dialogs - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^dolphin$|^org.kde.dolphin$", name=dlgs_Dolphin_Enter_is_Enter_Str)(ctx)
)
...
keymap("Overrides for Pantheon - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^io.elementary.files$")(ctx) )
...
keymap("Overrides for Krusader - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^org.kde.krusader$|^krusader$")(ctx) )
...
keymap("Overrides for Nautilus Create Archive dialog - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^org.gnome.nautilus$|^nautilus$", name="Create Archive")(ctx) )
...
keymap("Overrides for Nautilus - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^org.gnome.nautilus$|^nautilus$")(ctx) )
...
keymap("Overrides for PCManFM-Qt - Finder Mods - LXQt desktop", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^pcmanfm-qt$", name="^pcmanfm-desktop.*$")(ctx) )
keymap("Overrides for PCManFM-Qt - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^pcmanfm-qt$")(ctx) )
...
keymap("Overrides for PCManFM - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^pcmanfm$|^pcmanfm-qt$")(ctx) )
...
keymap("General File Managers - Finder Mods", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas=filemanagerStr)(ctx) )
...
keymap("GNOME Text Editor", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^gnome-text-editor$|^org.gnome.TextEditor$")(ctx) )
...
keymap("COSMIC Terminal overrides", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^com.system76.CosmicTerm$")(ctx) )
...
keymap("Terminology terminal", {
    ...
    cnfg.screen_has_focus and
    matchProps(clas="^terminology$")(ctx) )
