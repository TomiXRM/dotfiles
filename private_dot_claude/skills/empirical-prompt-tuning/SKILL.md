...
description: agent 向けテキスト指示（skill / slash command / task プロンプト / CLAUDE.md 節 / コード生成プロンプト）を、バイアスを排した実行者に動かしてもらい、両面（実行者の自己申告 + 指示側メトリクス）で評価して反復改善する手法。改善が頭打ちになるまで回す。プロンプトや skill を新規作成・大幅改訂した直後、またはエージェントの挙動が期待通りにならない原因を指示側の曖昧さに求めたいときに使う。本文(body)の明瞭性を評価する手法であり、どの skill が発火するか（description のルーティング）は別テストで確認する。高重要度では別モデルファミリでのクロスレビューを併用する。
---
...
- コストと釣り合わない規模: 1 ラウンド = シナリオ数 ×(必要なら +クロスモデル) 体の dispatch。skill 群をまとめて回すと数十体になりうる。重要度に応じて scenarios 数・round 数・クロスモデルの有無をスケールさせる（量産の reps は安く、設計レビューは厚く）。

### このskillが測れること / 測れないこと

- **測れる**: body/本文の明瞭性・自己完結性（実行者が指示に従って成果物を出せるか）。
- **測れない**: description によるルーティング（正しい skill が発火するか）。これは別テストが要る。
  対象群の **frontmatter（description）だけ** を実行者に与え、代表的なユーザ発話に対して「最初に発火させる単一 skill」を選ばせる。router / サブスキル構成では、曖昧な入口発話で router が先に発火するか（ゲート迂回が起きないか）、明示的な要求で正しいサブスキルが直接発火するかをこれで確認する。content 評価は「正しい skill に着地した後」しか見られないので、発火層の欠陥（description のトリガー競合）は素通りする。
...
5. **差分適用**: 不明瞭点を潰す最小修正をプロンプトに入れる。1 イテレーション 1 テーマ（関連する複数修正は OK、無関係な修正は次回に回す）。
   ...
   - **収束した多源証拠は「1テーマ厳格」の例外**: 独立した複数レビュア（別シナリオの runner 群 / 設計レビュー / クロスモデル）が **同一の指摘に収束** した場合は、帰属がすでに立っているのでまとめて修正し、直後の再テストで回帰を検出してよい。「1テーマ厳格」は情報源が単一で帰属が不確かなときの規律。逆に、単一の不明瞭点を別軸の修正と束ねるのは禁止（何が効いたか追えなくなる）。
6. **再評価**: 新しい subagent で再度 2 → 5 を回す（同一 agent は再利用しない: 前回の改善を学習している）。並列度はイテレーションを進めても改善が頭打ちにならない場合に増やす。
7. **収束判定**: 目安「連続 2 イテレーションで新規の不明瞭点ゼロ かつ メトリクス改善が閾値以下」で停止（具体的閾値は「反復の打ち切り基準」節に一元定義）。重要度が高いプロンプトは 3 連続にする。
...
- **計測の注意**: 対象プロンプトを subagent に Read させると、その Read 自体が `tool_uses` に乗り references-descent シグナルを汚す。相対比較したいなら起動契約で対象を **inline で貼る**（baseline を「初期 Read 0」に揃える）。

精度 100% でも `tool_uses` の偏りがあれば iter 2 発動の根拠になる。「精度のみで判断して打ち切り」は構造的欠陥を見逃しがち。

### クロスモデルレビュー（高重要度の指示で推奨）

同一モデルの fresh dispatch は、そのモデル **共通の盲点** を共有する。「過剰さ・冗長さ・特定の解釈癖」を実行者が問題と感じない場合、何度 dispatch しても ○ が返り、欠陥が指標に出ない。重要度の高い指示では、**別モデルファミリ**（例: Codex/GPT、別世代）に同じ対象を 1 回レビューさせると、同族 dispatch では出ない指摘が出る。

- 使い分け: 量産の reps（精度・`tool_uses` の反復計測）は同一モデルで安く回す。設計レビュー（抜け・過剰・発火競合）には 1 回クロスモデルを混ぜる。
- 効き方の実例: 「小さなタスクに重い手順を課す過剰適用」は同族 runner が ○ と見なしがちだが、別モデルは即 P1 として挙げることがある。
- 扱い: クロスモデルの指摘も「両面評価」と同じく、不明瞭点として要件チェックリスト / 判定文言に紐付けてから差分適用する（モデルが違っても規律は同じ）。
...
## 対象プロンプト
<対象プロンプトの本文を全文 inline で貼る（推奨）。Read で読ませるパス指定でも可だが、
 その Read が tool_uses に乗るので、tool_uses を相対比較するなら inline に寄せる>
...
| 「複数の不明瞭点を一気に潰そう」                      | 単一情報源なら何が効いたか分からなくなる。1 イテレーション 1 テーマ。（複数レビュアが収束した指摘は例外＝バッチ可） |
...
| 「精度が全シナリオ ○ だから完成」                     | 同一モデルは共通の盲点を ○ と返す。高重要度は別モデルで 1 回クロスレビューする。                                     |
...
- **イテレーションごとに変更多すぎ**: 「あのときの修正のどれが効いたか」が追えなくなる。1 修正 1 イテレーション（収束した多源証拠は除く）
- **シナリオを修正に合わせてチューニング**: 不明瞭点が潰れたように見せるため、シナリオ側を簡単にする → 本末転倒
- **content だけ評価して dispatch を測らない**: body が完璧でも、description のトリガー競合で別 skill が先に発火すればゲートは無効化される。「測れること / 測れないこと」節の発火テストを別建てで回す

## 関連

> 以下は外部 skill。手元に無ければ無視してよい（死リンクではなく参照ポインタ）。
