...
- **`[( ...]`とバッククォートのコード記法 `` `...` `` を同じ`[( ]`の中に入れない**（Scrapbox本体のパーサ制限。両方を1つの装飾に混ぜると `[(`/`]` がただの文字として表示され装飾ごと壊れる。実機DOM調査で確認済み、詳細は[chezmoi CI]ページ）。ただし**コード記法を`[( ]`の外に出して挟めば同じ行に共存できる**——地の文の区間ごとに`[( ]`で個別に包み、コード部分はその外側の素のバッククォートのまま置く。1行に`[( ]`を複数回使ってよい
  - 例: `` [( CIランナー上にage鍵が存在しないため]`chezmoi apply`[( が暗号化ファイルの復号を試みて失敗する] ``
  - 1行全体がコードだけ（前後に地の文が無い）場合は、囲む地の文が無いので`[( ]`は不要、素のバッククォートだけでよい
- 長い1行に詰め込まず、行頭スペースでの階層(ネスト)を徹底する。子行を含めて**地の文の区間は全部`[( ]`で包む**（「ラベル: 説明文1。説明文2」のように1行に詰め込まない。ラベル単独の装飾行を親にし、説明文は1文ずつ子行(行頭スペース2個)にする）
    ```
     [( 原因]
      [( [age暗号化]で秘密情報を管理する仕組み(PR #35)を入れた]
      [( CIランナー上にage鍵が存在しないため]`chezmoi apply`[( が復号を試みて失敗する]
    ```
    (`[( 原因: ...]`と1行に詰め込まない。実例は[chezmoi CI]ページ)
